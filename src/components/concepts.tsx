...
import { conceptData } from "@/lib/config";
...
type Projection = (typeof conceptData)[number]["projection"];
 ...
 */
export function Concepts({ t }: { t: Dictionary }) {
  const c = t.concepts;
  ...
  const onKeyDown = (event: React.KeyboardEvent) => {
    const last = conceptData.length - 1;
    let next = active;
    ...
    <section id="concepts" className="relative scroll-mt-24 border-t border-[var(--rule)]">
      <div className="mx-auto w-full max-w-[var(--shell)] px-[var(--gutter)] py-[clamp(3.25rem,6.5vh,5rem)]">
        ...
        >
          ...
          >
            {conceptData.map((data, i) => {
              const selected = i === active;
              const copy = c.items[i];
              return (
                <button
                  key={data.ref}
                  ref={(el) => {
                ...
                >
                  <span
                    ...
                    aria-hidden="true"
                  />
                  <span className="ms-mono block text-gold-400">
                    {c.conceptWord} {data.ref}
                  </span>
                  <span
                  ...
                  <span className="ms-mono mt-2 block text-[0.5625rem]">
                    {data.followers} · {data.platform}
                  </span>
                  <span className="ms-figure ms-money mt-2.5 block text-[1.0625rem]">
                    {money.format(project(data.projection).revenue)}
                  </span>
                </button>
              );
            })}
          ...
          {conceptData.map((data, i) => (
            ...
            >
              {i === active ? <Sheet index={i} t={t} /> : null}
            </div>
          ))}
        </div>
...
function Sheet({ index, t }: { index: number; t: Dictionary }) {
  const c = t.concepts;
  const data = conceptData[index];
  const copy = c.items[index];

  return (
    <div key={data.ref} className="ms-rise">
      <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2 border-b border-[var(--rule)] px-6 py-5 sm:px-9">
        <p className="ms-display flex flex-wrap items-baseline gap-3 text-[clamp(1.15rem,1.9vw,1.5rem)]">
          <span className="text-ash">{copy.handle}</span>
          <span className="text-gold-600" aria-hidden="true">
            →
          </span>
          <span className="ms-gold">{data.brand}</span>
        </p>
        <p className="ms-mono text-[0.5625rem]">{copy.field}</p>
      </div>

      <ProjectionBand index={index} t={t} />

      <div className="grid sm:grid-cols-2">
        <div className="border-b border-[var(--rule)] px-6 py-7 sm:border-b-0 sm:border-r sm:px-9 sm:py-9">
          <p className="ms-mono">{c.beforeLabel}</p>
          <p className="mt-5 text-[0.9375rem] leading-[1.7] text-ash">{copy.beforeSummary}</p>
          <ul className="mt-6 space-y-2.5">
            {copy.beforePoints.map((point) => (
        ...
        <div className="px-6 py-7 sm:px-9 sm:py-9">
          <p className="ms-mono text-gold-400">{c.afterLabel}</p>
          <p className="ms-display mt-5 text-[1.5rem] text-bone">{data.brand}</p>
          <p className="ms-mono mt-1.5 text-[0.5625rem]">{copy.line}</p>
          <p className="mt-5 text-[0.9375rem] leading-[1.7] text-bone">{copy.afterSummary}</p>
      ...
      <dl className="grid border-t border-[var(--rule)] sm:grid-cols-3">
        <div className="border-b border-[var(--rule)] px-6 py-5 sm:border-b-0 sm:border-r sm:px-9">
          <dt className="ms-mono">{c.ladderLabel}</dt>
          <dd className="ms-money mt-2 text-[0.875rem] font-medium leading-relaxed">
            {copy.ladderTiers
              .map((tier, i) => `${tier} ${money.format(data.ladderPrices[i])}`)
              .join(" · ")}
          </dd>
        </div>
        <div className="border-b border-[var(--rule)] px-6 py-5 sm:border-b-0 sm:border-r sm:px-9">
          <dt className="ms-mono">{c.modelLabel}</dt>
          <dd className="mt-2 text-[0.875rem] leading-relaxed text-bone">{copy.model}</dd>
        </div>
        <div className="px-6 py-5 sm:px-9">
          <dt className="ms-mono">{c.horizonLabel}</dt>
          <dd className="mt-2 text-[0.875rem] leading-relaxed text-bone">{copy.horizon}</dd>
        </div>
      </dl>
    </div>
  );
 ...
 */
function ProjectionBand({ index, t }: { index: number; t: Dictionary }) {
  const c = t.concepts;
  const data = conceptData[index];
  const p = data.projection;
  const { engaged, buyers, revenue } = project(p);
  const shown = useTicker(revenue, data.ref);

  const workings = [
    { label: c.audienceRow, value: count.format(p.audience) },
    { label: c.engagedRow, value: `${count.format(engaged)} · ${p.engagement}%` },
    { label: c.buyersRow, value: `${count.format(buyers)} · ${(p.conversion * 100).toFixed(1)}%` },
    { label: c.priceRow, value: money.format(p.price) },
  ];
  ...
  return (
    <div className="grid border-b border-[var(--rule)] lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <div className="border-b border-[var(--rule)] px-6 py-6 sm:px-9 lg:border-b-0 lg:border-r">
        <p className="ms-mono text-[0.5625rem]">{c.projectedLabel}</p>
